...
class LTCDataset(Dataset): 
    def create_dataset(self, x, x_comps, spikes, true_rates, trial_length, dt, num_trials): 
        # construct data dictionary from simulated spikes
        data_dict={'spikes':spikes, 'lowD':x, 'true_rates':true_rates, 'lowD_components': x_comps}
        ...
        start_times = np.linspace(0, spikes.shape[0]-(trial_length/dt), num_trials)
        end_times = start_times + (trial_length/dt) - 1
        ...
        self.init_data_from_dict(data_dict=data_dict, sample_interval=dt, trial_info=trial_info)
...
if __name__ == "__main__":
    ...
    trial_length = 2 # length of each trial, s 
    dt = 0.02 # timestep, s
    num_trials = 30 # number of trials to simulate 
    amplitudes = list(range(1,9)) # oscillator amplitude 
    num_neurons = 100 # dimension of the high D space
    valid_ratio = 0.2

    train_ratio = 1 - valid_ratio
    total_trials = len(amplitudes)*num_trials
    ...
    train_inds = inds[:int(train_ratio * total_trials)]
    valid_inds = inds[int(train_ratio * total_trials):]
    train_data = spikes_trials[train_inds, :, :]
    valid_data = spikes_trials[valid_inds, :, :]

    # save LFADS data 
    # need keys ['train_data', 'train_inds', 'valid_data', 'valid_inds']
    # data should be trials x time x neurons 
    # inds should be 1 x trials 
    savepath = '/snel/home/brianna/projects/deep_learning_project/lfads_nonlinearOscillator.h5'
    hf = h5py.File(savepath, 'w')
    hf.create_dataset('train_data', data=train_data)
    hf.create_dataset('valid_data', data=valid_data)
    hf.create_dataset('train_inds', data=np.expand_dims(train_inds, axis=0))
    hf.create_dataset('valid_inds', data=np.expand_dims(valid_inds, axis=0))
    hf.close()

    # # save the RDS dataset - currently broken/cannot merge to it 
    lfads_save_path = '/snel/home/brianna/projects/deep_learning_project/'
    chop_params = {'trial_length_s':2, 'trial_olap_s':0, 'bin_size_s': 0.02}      # not chopping data
    d = LTCDataset(name='nonlinearOscillator')
    d.create_dataset(x, x_comps, spikes, true_rates, trial_length, dt, total_trials)
    # d.make_trials(margins = 0.0, start_name='trialStart', end_name='trialEnd')
    # d.select_trials('good', {'good':'good'})
    d.init_lfads(lfads_save_path, lfads_save_path, chop_params, valid_ratio)
    d.elf.train_inds = train_inds
    d.elf.valid_inds = valid_inds
